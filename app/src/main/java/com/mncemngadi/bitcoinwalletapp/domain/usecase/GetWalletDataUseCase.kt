...
class GetWalletDataUseCase @Inject constructor(
    ...
    operator fun invoke(): Flow<Either<Failure, WalletData>> {
        ...
        return combine(btcAmountFlow, flow {
            ...
            val fluctuationResult = repository.getFluctuation("EUR", symbols, yesterdayStr, today)
            
            val finalResult = result.map { rates ->
                ...
                val btcBaseRates = rates.map { rate ->
